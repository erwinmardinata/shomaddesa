...
class Pengumuman extends Back_controller {

	function __construct() {

		parent::__construct();
		$this->load->model('Mpengumuman', 'mdl');
	...
	function get_data() {
		...
		foreach($fetch_data as $row)
		{
				 ...
				 $sub_array[] = $no++;
				 $sub_array[] = $row->nama;
				 $sub_array[] = '<a href="'.site_url('Pengumuman/edit/'.$row->id).'" class="btn btn-info btn-xs update">Edit</a>
				 <a href="'.site_url('Pengumuman/delete/'.$row->id).'" onclick="return confirm(\'Anda yakin?\')" class="btn btn-danger btn-xs delete">Delete</a>';
				 $data[] = $sub_array;
		}
		$output = array(
