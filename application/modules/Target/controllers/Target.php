...
class Target extends Back_controller {

	function __construct() {

		parent::__construct();
		$this->load->model('Mtarget', 'mdl');
	...
	function get_data() {
		...
		foreach($fetch_data as $row)
		{
				 ...
				 $sub_array[] = $row->nama;
				 $sub_array[] = $row->tahun;
				 $sub_array[] = number_format($row->target, 0, ".", ".");
				 $sub_array[] = '<a href="'.site_url('Target/edit/'.$row->id).'" class="btn btn-info btn-xs update">Edit</a>
				 <a href="'.site_url('Target/hapus/'.$row->id).'" onclick="return confirm(\'Anda Yakin?\')" class="btn btn-danger btn-xs delete">Delete</a>';
				 $data[] = $sub_array;
		}
		$output = array(
